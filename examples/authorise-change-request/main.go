...
func exec(isApproved bool) error {
	...
	we, err := c.ExecuteWorkflow(ctx, workflowOptions, "authoriseChangeRequest", dsl.HTTPData{
	...
	go func() {
		...
		dur := time.Second * 30
		log.Info().Dur("timeout", dur).Bool("isApproved", isApproved).Msg("Waiting before we trigger review")
		time.Sleep(dur)

		log.Info().Msg("Sending change review response")
		if err := c.SignalWorkflow(ctx, we.GetID(), "", "review", &dsl.HTTPData{
...
func main() {
	...
	fmt.Println("Do you want to simulate approval of the workflow? (Y/n)")
